...
# dictionary for execution time (gurobi) Key = value of alpha
times_gurobi = {alpha : [] for alpha in val_alpha}  
...
for i, data in enumerate(gpu_data):
    ...
    for alpha in val_alpha:
        ...
        # --- GUROBI ---
        # measuring only Gurobi
        t_gurobi_start = time.perf_counter()

        # QUBO solution  
        sol_gurobi = qubo_solver.qubo(Q)

        duration_gurobi = time.perf_counter() - t_gurobi_start
        times_gurobi[alpha].append(duration_gurobi)
...
fig1.savefig('metrics_gurobi_case1.png', dpi=300)
plt.close(fig1)

# TIMES PLOTS GUROBI with slicing
...
# total time for gurobi
total_g = [np.sum(times_gurobi[o]) for o in val_alpha]
...
plt.title('Average Time Gurobi per Image (3 Densities)')
plt.xlabel('$\\alpha$')
plt.ylabel('Seconds')
plt.legend()
plt.grid(True, alpha=0.3)
...
# total times
plt.subplot(1, 2, 2)
plt.plot(val_alpha, total_g, 'o-', label='Tot Gurobi (All Images)', color='black')
plt.xticks(val_alpha, rotation=45)
...
plt.title('Total Execution Time Gurobi')
plt.xlabel('$\\alpha$')
plt.ylabel('Seconds')
plt.legend()
plt.grid(True, alpha=0.3)
...
plt.tight_layout()
fig2.savefig('times_gurobi_case1.png', dpi=300)
plt.close(fig2)
...
table_dens.field_names = ["Density", "Range Box", "N. Images", "mAP @ Best alpha with Gurobi"]
...
fig1.savefig('metrics_gurobi_case2.png', dpi=300)
plt.close(fig1)
...
plt.tight_layout()
fig2.savefig('times_gurobi_case2.png', dpi=300)
plt.close(fig2)
...
fig1.savefig('metrics_gurobi_case3.png', dpi=300)
plt.close(fig1)
...
plt.tight_layout()
fig2.savefig('times_gurobi_case3.png', dpi=300)
plt.close(fig2)
...
fig1.savefig('metrics_gurobi_case4.png', dpi=300)
plt.close(fig1)
...
plt.tight_layout()
fig2.savefig('times_gurobi_case4.png', dpi=300)
plt.close(fig2)
